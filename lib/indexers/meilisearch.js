...
module.exports = function meilisearch(config) {
	...
	async function updateIndexSettings(collection, collectionSettings) {
		try {
			if (!collectionSettings) return;

			await axios.patch(`${config.host}/indexes/${collection}/settings`, collectionSettings, axiosConfig);
		} catch (error) {
			if (error.response) {
	...
	async function deleteItems(collection) {
		try {
			...
			await axios.delete(`${config.host}/indexes/${collection}/settings`, axiosConfig);
		} catch (error) {
