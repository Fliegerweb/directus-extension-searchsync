 ...
 */
function registerHook({ action, filter, init }, { services, env, database, logger, getSchema }) {
	...
	action('server.start', () => {
		if (!extensionConfig.reindexOnStart && env.REINDEX_SEARCH !== true) return;
	...
	filter('items.delete', (keys, { collection }) => {
		indexer.preDeleteItemIndex(collection, keys);
