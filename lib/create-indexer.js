...
const deleteCache = new NodeCache({ stdTTL: 60 });
...
function createIndexer(config, { logger, database, services, getSchema }) {
	 ...
	 * @param {string[]} ids
	 */
	async function preDeleteItemIndex(collection, ids) {
		const schema = await getSchema();

		/**
		 * @type {Map<string, string[]>}
		 */
		let collectionsToDelete = new Map();

		if (!config.collections.hasOwnProperty(collection)) {
			// Check if the collection is related to any configured collections
			collectionsToDelete = await getRelatedCollections(schema, collection, ids, true);
		} else {
			collectionsToDelete.set(collection, ids);
		}
		...
		for await (const [collection, ids] of collectionsToDelete) {
			...
			itemsToDelete.set(collection, items);
		}

		deleteCache.set(`${collection}_${ids.join()}`, itemsToDelete);
	}
	...
	async function deleteItemIndex(collection, ids) {
		...
		/**
		 * @type {Map<string, Record<string, any>[]> | undefined}
		 */
		let itemsToDelete = deleteCache.get(`${collection}_${ids.join()}`);

		if (!itemsToDelete) {
			return;
		}

		itemsToDelete.forEach(async (items, collection) => {
			const collectionIndex = getCollectionIndexName(collection);
	...
	async function updateItemIndex(collection, ids) {
		...
		if (!config.collections.hasOwnProperty(collection)) {
			// Check if the collection is related to any configured collections
			collectionsToUpdate = await getRelatedCollections(schema, collection, ids, false);
		} else {
	...
	async function getRelatedCollections(schema, collection, ids, cascadeDelete) {
		...
		for await (const relatedCollection of collectionRelations) {
			if (configuredCollections.includes(relatedCollection.collection)) {
				...
				if (fields && !fields.find((e) => {
					return e === relatedCollection.schema.column || e.startsWith(`${relatedCollection.schema.column}.`)
				})) {
					continue;
				}

				// Check on delete cascade entries
				if (cascadeDelete) {
					if (relatedCollection.schema.on_delete !== "CASCADE") {
						continue;
					}
				}
